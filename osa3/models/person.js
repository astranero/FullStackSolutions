...
personSchema.static('readPersons', function () {
  return Person.find({})
    .then((persons) => {
...
personSchema.static('addPerson', function (personData) {
...
personSchema.static('deletePerson', function (id) {
  return Person.findByIdAndDelete(id)
    .then((deletedPerson) => {
      if (deletedPerson) {
        console.log(
          `Deleted ${deletedPerson.name} number ${deletedPerson.number} from phonebook`
        )
...
personSchema.static('updatePerson', function (personData) {
  return Person.findByIdAndUpdate({ _id: personData.id }, personData, {
    new: true,
  })
    .then((updatedPerson) => {
...
personSchema.static('getById', function (id) {
